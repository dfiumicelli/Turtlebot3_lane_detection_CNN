# dataset_ROBOT.py - Ottimizzato per 640x480
...
class LaneSegmentationDataset(Dataset):
    """
    ✅ Dataset ottimizzato per ROBOT 640x480
    
    Ridimensiona 1280x720 → 640x480 (aspect ratio preservato!)
    """
    ...
    def __init__(self, images_dir, masks_dir, test_images_dir=None, test_masks_dir=None, 
        ...
        self.img_size = img_size  # 640x480 per robot
    ...
    def __getitem__(self, idx):
        ...
        if image is None:
            print(f"⚠️ Non riesco a leggere: {self.images_fps[idx]}")
        ...
        if mask is None:
            print(f"⚠️ Non riesco a leggere: {self.masks_fps[idx]}")
            return self.__getitem__(0)
        
        # ⭐ RIDIMENSIONA a 640x480 (aspect ratio preservato!)
        # Original: 1280x720 → Target: 640x480 (esattamente dimezzato)
