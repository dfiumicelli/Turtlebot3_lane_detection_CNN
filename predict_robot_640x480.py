# predict.py - VERSIONE CORRETTA PER ROBOT 640x480
...
class LanePredictor:
    """Classe per fare predizioni con U-Net addestrato - ROBOT 640x480"""
    ...
    def __init__(self, model_path, encoder='mobilenet_v2', encoder_weights=None, device=None):
        ...
        Args:
            ...
            encoder_weights (str): Pesi encoder usati durante training (None per trained model)
        ...
        # ⭐ ROBOT INPUT SIZE: 640x480 (NO RESIZE!)
        ...
        self.model = smp.Unet(
            ...
            encoder_weights=None,  # Non caricare pesi ImageNet (già addestrato)
            classes=1,
            activation=None,  # Aggiungi sigmoid manualmente
        )
        ...
        try:
            ...
            # Gestisci sia dict che state_dict diretto
        ...
        # ⭐ PIPELINE PREPROCESSING per ROBOT 640x480
        # Stesso preprocessing usato nel training!
        self.transform = A.Compose([
            # ⭐ RESIZE FISSO a 640x480 (come nel training)
        ...
        print(f"   Input size: {self.input_width}x{self.input_height} (robot compatible)")
    ...
    def predict(self, image_path, threshold=0.5, return_original=True):
        ...
        # ⭐ PREPROCESSING: Resize a 640x480 + normalizzazione
...
def main():
    ...
    MODEL_PATH = 'test08_nn/best_unet_mobilenet2.pth'
