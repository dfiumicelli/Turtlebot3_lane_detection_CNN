#!/usr/bin/env python3
"""
Predictor per U-Net 3-CLASSI CON CANALE HSV - FIXED
✓ Supporta PyTorch (.pth) e ONNX (.onnx) con 4 canali!
✓ Input: RGB + Hue (4 canali) come nel training
✓ Output: 3 classi (BG, Bianca, Gialla)
✓ FIX: Ridimensiona Hue PRIMA di concatenare!
"""
...
class LanePredictor3ClassHSV:
    """Predictor U-Net 3-classi con canale HSV"""
    ...
    def __init__(self, model_path, encoder='mobilenet_v2', device=None):
        ...
        self.input_width = 240
        self.model = None
        self.session = None
        self.model_type = None

        self.num_classes = 3
        self.class_names = ['Background', 'Continua (Bianca)', 'Discontinua (Gialla)']
        self.class_colors = {
            0: (0, 0, 0),
            1: (255, 255, 255),
            2: (0, 255, 255)
    ...
    def _load_pytorch_model(self, model_path, device=None):
        """Carica modello PyTorch (.pth) con 4 canali"""
    ...
    def _load_onnx_model(self, model_path, device=None):
        """Carica modello ONNX (.onnx) con 4 canali"""
    ...
    def predict(self, image_path, return_original=True):
        """Predizione con 4 canali (RGB + Hue)"""
        ...
        # ⭐ PREPROCESSING RGB (ridimensiona a 240x320)
        sample = self.transform(image=original_image)
        image_rgb = sample['image']  # (3, 240, 320)

        # ⭐ AGGIUNGI CANALE HUE
        # IMPORTANTE: Estrai da immagine RIDIMENSIONATA, non originale!
        image_uint8 = (original_image * 255).astype(np.uint8) if original_image.max() <= 1 else original_image.astype(
            np.uint8)
        image_hsv = cv2.cvtColor(image_uint8, cv2.COLOR_RGB2HSV)

        # ⭐ RIDIMENSIONA HSV PRIMA di estrarre il canale
        h_w = self.input_width
        h_h = self.input_height
        image_hsv_resized = cv2.resize(image_hsv, (h_w, h_h), interpolation=cv2.INTER_LINEAR)

        h_channel = image_hsv_resized[:, :, 0].astype(np.float32) / 180.0
        h_tensor = torch.from_numpy(h_channel).unsqueeze(0)  # (1, 240, 320)

        # ⭐ VERIFICA DIMENSIONI
        assert image_rgb.shape == (3, self.input_height, self.input_width), \
            f"RGB shape mismatch: {image_rgb.shape}"
        assert h_tensor.shape == (1, self.input_height, self.input_width), \
            f"Hue shape mismatch: {h_tensor.shape}"

        # ⭐ CONCATENA RGB + H = 4 canali
        image_with_hue = torch.cat([image_rgb, h_tensor], dim=0)  # (4, 240, 320)

        # Inference
        if self.model_type == 'pytorch':
            image_with_hue = image_with_hue.unsqueeze(0).to(self.device).float()
            with torch.no_grad():
                prediction = self.model(image_with_hue)

            class_map = torch.argmax(prediction, dim=1).squeeze().cpu().numpy()

        else:  # ONNX
            image_with_hue_np = image_with_hue.unsqueeze(0).numpy().astype(np.float32)
            outputs = self.session.run(None, {'input': image_with_hue_np})
    ...
    def visualize(self, image_path, save_path=None):
        ...
        fig.suptitle(f'Lane Segmentation 3-Class + HSV ({self.model_type.upper()}): {os.path.basename(image_path)}',
                     fontsize=16)
        ...
        axes[0, 1].set_title('Segmentazione 3-Classi (RGB + Hue)')
        axes[0, 1].axis('off')
    ...
    def _save_complete_visualization(self, original_image, class_map, base_name, output_dir):
        ...
        cv2.putText(combined, 'Segmentazione 3-Class (RGB+Hue)', (w + 20, 30), font, font_scale, color, thickness)
        ...
        cv2.putText(combined, '- Giallo: Discontinua', (20, legend_y + 90), font, 0.6, (0, 255, 255), 1)
...
def main():
    ...
    # ⭐ MODELLO CON 4 CANALI!
    MODEL_PATH = 'best_unet_3class.onnx'  # ← Modello ONNX 3-classi

    # Oppure usa il modello PyTorch
    # MODEL_PATH = 'best_unet_3class.pth'  # ← Modello PyTorch 3-classi
    ...
    print("\n" + "=" * 80)
    print("🚀 ROBOT LANE SEGMENTATION (3-CLASSI + HSV)")
    print("=" * 80 + "\n")

    predictor = LanePredictor3ClassHSV(
